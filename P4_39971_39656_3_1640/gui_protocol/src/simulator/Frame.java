...
public class Frame {
    ...
    /**
     * Creates a new instance (object) of an Nack frame
     * @param nack acknowledgment number
     * @return the frame object created
     */
    public static Frame new_Nack_Frame(int nack) {
        Frame frame= new Frame();
        frame.set_NACK_frame(nack);
        return frame;
    }
    ...
    public String toString() {
        ...
        if (kind == DATA_FRAME || kind == ACK_FRAME || kind == NACK_FRAME) {
    ...
    /**
     * Used with NACK_FRAME frames to set the fields values
     * @param ack missing sequence number
     * @return true if successful, false otherwise
     */
    public boolean set_NACK_frame(int nack) {
        if (nack <= UNDEFINED_SEQ) {
            return false;
        }
        kind = NACK_FRAME;
        this.ack = nack;
    ...
    /**
     * Acknowledge number - for DATA, ACK and NACK packets; it defines the 
     * sequence number of the last DATA frame successfully received for DATA and 
     * ACK; if defines the missing DATA frame for NACK packets
     */
    private int ack;
